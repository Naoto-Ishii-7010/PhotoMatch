...
- ルート: `/lp`
- ファイル: `app/lp/layout.tsx`, `app/lp/page.tsx`
- コンポーネント: `components/features/lp/`
- デザインテイスト: クリーム/ベージュ背景（#F8F5EF）・ゴールドアクセント（#C9A96E）・セージグリーン（#7FB5A0）
- セクション構成: ヒーロー → サービス概要 → ターゲット → 利用の流れ → 機能紹介 → 安心・安全 → CTA
